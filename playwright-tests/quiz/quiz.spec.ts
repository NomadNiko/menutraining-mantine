...
test.describe('Quiz Functionality', () => {
  ...
  test('High scores display', async ({ page }) => {
    ...
    await expect(page.locator('text="Restaurant ID: RST-000001"')).toBeVisible();
  ...
  test('Start and play quiz', async ({ page }) => {
    ...
    const questionElements = await page.locator('h1, h2, h3, p').allTextContents();
    const hasQuestion = questionElements.some(text => 
      text.includes('?') || 
      text.includes('Which') || 
      text.includes('What') || 
      text.includes('Does')
    ...
    const trueOption = page.locator('label:has-text("True"), input[value="true"]').first();
    const falseOption = page.locator('label:has-text("False"), input[value="false"]').first();
  ...
  test('Quiz configuration page elements', async ({ page }) => {
    ...
    await expect(page.locator('text="Quiz Configuration"')).toBeVisible({ timeout: 10000 });
