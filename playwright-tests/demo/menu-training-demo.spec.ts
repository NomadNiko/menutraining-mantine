...
test.describe('Menu Training Application Demo', () => {
  test('Complete application walkthrough', async ({ page }) => {
    ...
    await test.step('Verify restaurant data is loaded', async () => {
      ...
      const ingredientElements = await page.locator('tr, [class*="card"]').count();
      expect(ingredientElements).toBeGreaterThan(0);
      
      // Check menu items
      await page.goto('/en/restaurant/menu-items');
      await page.waitForLoadState('networkidle');
      const menuItemElements = await page.locator('tr, [class*="card"]').count();
    ...
    await test.step('Quiz system', async () => {
      ...
      await expect(page.locator('h1, h2').first()).toContainText('Menu Training Quiz');
    ...
    await test.step('Documentation screenshots', async () => {
      ...
      await page.goto('/en/restaurant/quiz');
      await page.screenshot({ path: 'playwright-tests/screenshots/quiz-config.png', fullPage: true });
      
      // Ingredients page
      await page.goto('/en/restaurant/ingredients');
      await page.screenshot({ path: 'playwright-tests/screenshots/ingredients.png', fullPage: true });
      
      // Menu items page
      await page.goto('/en/restaurant/menu-items');
      await page.screenshot({ path: 'playwright-tests/screenshots/menu-items.png', fullPage: true });
    });
  });
  ...
  test('Quiz gameplay', async ({ page }) => {
    ...
    if (currentUrl.includes('/quiz/question')) {
      ...
      const question = questionText.find(text => 
        text.includes('?') || text.includes('contain') || text.includes('Which')
      );
      ...
      await page.screenshot({ path: 'playwright-tests/screenshots/quiz-question.png' });
    }
  });
});
