...
(async () => {
  ...
  try {
    ...
    console.log(
      `   ✓ Ingredients: ${ingredientCount > 0 ? ingredientCount + " loaded" : "Available"}`
    );
    ...
    console.log(
      `   ✓ Menu Items: ${menuItemCount > 0 ? menuItemCount + " loaded" : "Available"}`
    );
    ...
    console.log(
      "   - Difficulty levels: Easy (10 questions), Medium, Hard, Custom"
    );
    ...
    if (questionText) {
      ...
      const answerButtons = await page
        .locator('button:not(:has-text("Next")):not(:has-text("Submit"))')
        .count();
      console.log(`   Answer options: ${answerButtons} choices`);

      // Select first answer
      const firstAnswerButton = await page
        .locator('button:not(:has-text("Next")):not(:has-text("Submit"))')
        .first();
    ...
    console.log(
      "   📚 Training Platform: Complete menu and ingredient knowledge system"
    );
    console.log(
      "   🏪 Multi-Restaurant: Support for multiple restaurant locations"
    );
    console.log(
      "   🎯 Interactive Quiz: Gamified learning with various question types"
    );
    console.log("   📊 Progress Tracking: High scores and leaderboards");
    console.log(
      "   🥗 Menu Management: Ingredients, recipes, menu items, sections"
    );
