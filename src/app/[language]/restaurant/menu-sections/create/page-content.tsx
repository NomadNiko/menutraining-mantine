// src/app/[language]/restaurant/menu-sections/create/page-content.tsx
"use client";
import { Container, Paper } from "@mantine/core";
import { useRouter } from "next/navigation";
import { useTranslation } from "@/services/i18n/client";
import { useCreateMenuSectionService } from "@/services/api/services/menu-sections";
import { MenuSectionForm } from "@/components/menu-sections/MenuSectionForm";
import useGlobalLoading from "@/services/loading/use-global-loading";
import { useSnackbar } from "@/components/mantine/feedback/notification-service";
import HTTP_CODES_ENUM from "@/services/api/types/http-codes";
import { CreateMenuSectionDto } from "@/services/api/types/menu-section";
...
function CreateMenuSection() {
  ...
  const createMenuSectionService = useCreateMenuSectionService();
  const { isMobile, isTablet } = useResponsive();

  // Determine container size based on screen size
  const containerSize = isMobile || isTablet ? "xs" : "sm";

  const handleSubmit = async (formData: CreateMenuSectionDto) => {
    ...
    try {
      ...
      const { status } = await createMenuSectionService(dataWithRestaurant);
