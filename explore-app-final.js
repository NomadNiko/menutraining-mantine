...
(async () => {
  ...
  try {
    ...
    const hamburger = await page
      .locator(
        'button[aria-label*="menu"], button[aria-label*="navigation"], [class*="burger"]'
      )
      .first();
    ...
    const uniqueLinks = [
      ...new Set(allLinks.filter((link) => link.trim() && link.length < 30)),
    ];
    console.log(`   Available navigation: ${uniqueLinks.join(", ")}\n`);

    // Check user role
    console.log("3. USER PERMISSIONS");
    const hasAdminAccess = uniqueLinks.some((link) =>
      link.toLowerCase().includes("admin")
    );
    const hasRestaurantAccess = uniqueLinks.some(
      (link) =>
        link.toLowerCase().includes("restaurant") ||
        link.toLowerCase().includes("ingredient")
    );
    console.log(`   Admin access: ${hasAdminAccess ? "✓ Yes" : "✗ No"}`);
    console.log(
      `   Restaurant management: ${hasRestaurantAccess ? "✓ Yes" : "✗ No"}\n`
    );
    ...
    const ingredientCount = await page
      .locator('h2, h3, [class*="title"], [class*="card"]')
      .count();
    ...
    const userName = profileData.find(
    ...
    console.log(
      "   ✓ Features: Ingredients, Recipes, Menu Items, Menu Sections, Menus"
    );
    console.log("   ✓ Interactive Quiz system for staff training");
    console.log("   ✓ Multi-restaurant support with role-based access");
    console.log(
      `   ✓ Current user: aloha@ixplor.app${hasAdminAccess ? " (Admin)" : " (User)"}`
    );
  } catch (error) {
